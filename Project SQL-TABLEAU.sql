...
-- Precio promedio por type de vino
SELECT type, AVG(price) AS average_price
FROM wines_spa
GROUP BY type;

-- Rating promedio por type de vino
SELECT type, AVG(rating) AS average_rating
FROM wines_spa
GROUP BY type;

-- Precio promedio por region
SELECT region, AVG(price) AS average_price
FROM wines_spa
GROUP BY region;

-- Rating promedio por region
SELECT region, AVG(rating) AS average_rating
FROM wines_spa
GROUP BY region;

-- Ordenar del vino mas caro al mas barato por type, region y vino (etiqueta)
SELECT type, region, wine, MAX(price) AS max_price, MIN(price) AS min_price
FROM wines_spa
GROUP BY type, region, wine
ORDER BY max_price DESC;

-- Tipologias de vino + Wines con su precio maximo filtrado por acides
SELECT s.type, s.wine, s.acidity, s.price AS max_price_filtered_by_acidity
FROM wines_spa s
JOIN (
    SELECT type, MAX(price) AS max_price
    FROM wines_spa
    GROUP BY type
) t ON s.type = t.type AND s.price = t.max_price
ORDER BY s.acidity, s.price DESC;

-- Tipologias de vino + Wines con su precio maximo filtrado por body y rating
SELECT s.type, s.wine, s.body, s.price AS max_price_filtered_by_body
FROM wines_spa s
JOIN (
    SELECT type, MAX(price) AS max_price
    FROM wines_spa
